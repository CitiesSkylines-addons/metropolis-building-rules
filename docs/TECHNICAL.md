...
The Harmony postfix on `BuildingManager.GetRandomBuildingInfo` runs only when enabled, when the vanilla result is non-null, and when the game's `style` argument is zero. A cache keyed by service, subservice, level, width, depth and zoning mode avoids rescanning every loaded prefab on every spawn. `RuleEngine.Choose` keeps the vanilla asset if it satisfies both preferences; otherwise it picks the closest compatible height under the configured maximum and matching the optional name keyword. It falls back to vanilla when the pool is empty. The cache is cleared when a level unloads.

The build script compiles against local game and CitiesHarmony assemblies. It runs pure rule checks for the accepted asset, replacement, combined filters, and fallback. These tests do not prove that a city loads or that Harmony patches coexist with other mods.
...
La postfix Harmony su `BuildingManager.GetRandomBuildingInfo` si esegue solo se attiva, con risultato vanilla non nullo e argomento `style` uguale a zero. Una cache indicizzata per servizio, sottoservizio, livello, larghezza, profondità e modalità di zoning evita la scansione di tutti gli asset a ogni costruzione. `RuleEngine.Choose` conserva l’asset vanilla se rispetta entrambe le preferenze; altrimenti sceglie l’altezza compatibile più vicina entro il limite e con la parola richiesta. Se il gruppo è vuoto, conserva l’asset vanilla. La cache viene svuotata quando il livello si chiude.

Lo script compila contro le librerie locali di gioco e CitiesHarmony. Esegue controlli indipendenti su asset già valido, sostituzione, filtri combinati e fallback. I controlli non dimostrano che una città si carichi o che le patch Harmony convivano con altre mod.
